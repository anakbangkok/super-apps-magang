...
<div class="container my-5">
    ...
    <form action="{{ route('mentor.tim_sosmeds.update', $tim_sosmed->id) }}" method="POST" class="p-4 border rounded shadow-sm" style="background-color: #f9f9f9;">
        @csrf
        @method('PUT')

        <div class="mb-3">
            <label for="nama" class="form-label">Nama</label>
            <input type="text" class="form-control" id="nama" name="nama" value="{{ $tim_sosmed->user->name }}" required readonly>
        </div>
        ...
        <a href="{{ route('mentor.tim_sosmeds.index') }}" class="btn btn-secondary">Batal</a>
    </form>
</div>
@endsection
