...
@section('content')
    <div class="container">
        ...
        <div class="card shadow">
            ...
            <div class="border-bottom pb-3 mt-4 mx-3">
                <div class="row">
                    <div class="col-md-4">
                    ...
                    <div class="col-md-4">
                        ...
                        <select id="nama-filter" class="form-control select2" placeholder="Cari Nama...">
                            ...
                            @foreach ($tim_sosmeds->unique('user_id') as $tim_sosmed)
                        ...
                        @foreach ($tim_sosmeds as $tim_sosmed)
                            <tr class="text-center">
                                ...
                                <td>
                                    ...
                                    <a href="{{ route('mentor.tim_sosmeds.edit', $tim_sosmed->id) }}"
                                        class="btn btn-warning btn-sm button-action me-2" title="Edit">Edit</a>
                                    <form id="deleteTimSosmedForm{{ $tim_sosmed->id }}"
                                        action="{{ route('mentor.tim_sosmeds.destroy', $tim_sosmed->id) }}" method="POST"
                                        class="d-inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="btn btn-danger btn-sm button-action"
                                            data-bs-toggle="modal"
                                            data-bs-target="#confirmDeleteModal{{ $tim_sosmed->id }}">Hapus</button>
                                    </form>

                                    <!-- Modal Konfirmasi Penghapusan -->
                                    <div class="modal fade" id="confirmDeleteModal{{ $tim_sosmed->id }}" tabindex="-1"
                                        aria-labelledby="confirmDeleteModalLabel{{ $tim_sosmed->id }}" aria-hidden="true">
                                        <div class="modal-dialog">
                                            <div class="modal-content shadow">
                                                <div class="modal-header"
                                                    style="background-color: #f8d7da; color: #721c24;">
                                                    <h5 class="modal-title"
                                                        id="confirmDeleteModalLabel{{ $tim_sosmed->id }}">Konfirmasi
                                                        Penghapusan</h5>
        ...
        <script>
            $(document).ready(function() {
                ...
                $.fn.dataTable.ext.search.push(function(settings, data, dataIndex) {
                    ...
                    var date = tableRow.find('td:eq(4)').data('order'); // Ambil dari data-order
