...
<div class="container my-5">
    ...
    <!-- Form Edit Tim Web -->
    <form action="{{ route('mentor.tim_webs.update', $tim_web->id) }}" method="POST" class="p-4 border rounded shadow-sm" style="background-color: #f9f9f9;">
        @csrf
        @method('PUT')
        <div class="mb-3">
            <label for="nama" class="form-label">Nama</label>
            <input type="text" name="nama" value="{{ $tim_web->user->name }}" class="form-control" readonly>
        </div>
        ...
        <a href="{{ route('mentor.tim_webs.index') }}" class="btn btn-secondary">Batal</a>
    </form>
</div>
@endsection
