...
@section('content')
    <div class="container my-5">
        ...
        <form method="POST" action="{{ route('admin.users.update', $user->id) }}" class="p-4 border rounded shadow-sm"
            ...
            <div class="mb-3">
                ...
                <select class="form-control select2" id="instansi" name="instansi" required>
                    ...
                    @foreach ($instansis as $instansi)
                        <option value="{{ $instansi->id }}" {{ $instansi->id == $user->instansi_id ? 'selected' : '' }}>{{ $instansi->nama_instansi }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-3">
                ...
                <select class="form-select select2" id="mentor" name="mentor" required>
                    ...
                    @foreach ($mentors as $mentor)
                        <option value="{{ $mentor->id }}" {{ $mentor->id == $user->mentor_id ? 'selected' : '' }}>
                            {{ $mentor->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-3">
                ...
                value="{{ old('start_date', $user->start_date ? \Carbon\Carbon::parse($user->start_date)->format('Y-m-d') : '') }}">
            </div>
